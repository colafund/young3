...
$(function() {
  ...
  var menu             = $('#menu'),
      ...
      NFTsTrigger = $('#NFTs-trigger'),
      NFTsSection = $('#NFTs'),
      NFTsClose   = $('#NFTs-close'),
  ...
  NFTsTrigger.on("click", function(event) {
    event.preventDefault();
    NFTsSection.addClass('active animate-in');
  });
  ...
  NFTsSection.on("scroll", function() {
    header.addClass('smaller');
    if ($('#NFTs .inner__content').offset().top < -50) {
      header.addClass('smaller');
    } else {
      header.removeClass('smaller');
    }
